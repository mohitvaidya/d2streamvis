#  lib imports
import re
from tqdm import tqdm
from timer import timer
from os.path import isfile, join, isdir
import ffmpeg
from ast import literal_eval

# Main utility for trim and duration reduction
basepath = '/app'

def ffmpeg_func(video_id = None, duration = None, fps =None):
    (
    ffmpeg
    .input(f'{basepath}/{video_id}.mp4')
    .filter('trim', duration= duration)    
    .filter('fps', fps=fps, round='up')
    .output(f'{basepath}/{video_id}_.mp4')
    .run(overwrite_output= True)
    )
    print("process finished")

    with open(f'{basepath}/{video_id}_tmp.txt','w') as f:
        f.write(str({"trim_duration":duration, "fps":fps}))

def pre_process(video_id = None, trim_duration=10,fps=1 ):
    '''
    Parameters
    ----------
    video_id : e.g. 11234, should not include any extensions
    basepath : absolute or relative path, must not end with slash("/"), e.g. /users/mnt
                can be same as current work dir
    video_folder : Meta videos should be stored inside this folder, must followed by basepath, 
                    e.g. /user/mnt/{video_folder}
    out_folder = videos output should be stored inside this folder, must followed by basepath, 
                    e.g. /user/mnt/{out_folder}

    '''
 
    if isfile(f'{basepath}/{video_id}_.mp4'):
        print(f'file already exists')
        print(f'checking for fps and trim duration')
        with open(f'{basepath}/{video_id}_tmp.txt','r') as f:
            val_dict = literal_eval(f.read())
        prev_duration = val_dict['trim_duration']
        prev_fps = val_dict['fps']
        print(f'prev_duration is {prev_duration} and prev_fps is {prev_fps}')

        if trim_duration== prev_duration and fps==prev_fps:
            
            print(f"Found same file loading it ")
            
            return 0
            
        else:
            ffmpeg_func(video_id = video_id, duration=trim_duration, fps=fps)

    else:
        ffmpeg_func(video_id = video_id, duration=trim_duration, fps=fps)

            # os.remove(f'{basepath}/{video_folder}/{video_id}.mp4')

# pre_process(video_id='15341')
# if __name__=='__main__':
#     preProcess(video_id='1335', basepath='/data1/code_base/mnt_data/ODbatch/', video_folder='vids', out_folder='outf')
